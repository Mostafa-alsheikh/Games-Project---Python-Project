...
def next_turn(row,col):
    ...
    if game_btns[row][col]['text'] =="" and check_winner() == False:
        if player == players[0]:
           ...
           # switch player
                player = players [1] 
                label.config(text=(players[1] + "turn"),bg ='green')
            elif check_winner() == True:
                 label.config(text=(players[0] + "wins!"),bg='gold')
            elif check_winner () == 'tie':
                 label.config(text=("yalla bara"),bg='brown')
            ...
            if check_winner() == False:
             # switch player
                player = players [0] 
                label.config(text=(players[0] + "turn"),bg='yellow')
            elif check_winner() == True:
                 label.config(text=(players[1] + "wins!"),bg='gold')
            elif check_winner() == 'tie':
                 label.config(text=("yalla bara"),bg='brown')
def check_winner():
    ...
    for row in range(3): 
        if game_btns[row][0]['text'] == game_btns[row][1]['text'] == game_btns[row][2]['text'] != "":
            game_btns[row][0].config(bg='gold')
            game_btns[row][1].config(bg='gold')
            game_btns[row][2].config(bg='gold')
            return True
    # check all 3 vertical conditions
    for col in range(3): 
        if game_btns[0][col]['text'] == game_btns[1][col]['text'] == game_btns[2][col]['text'] != "":
            game_btns[0][col].config(bg='gold')
            game_btns[1][col].config(bg='gold')
            game_btns[2][col].config(bg='gold')
            return True
    # check diagonals conditions 
    if game_btns[0][0]['text'] == game_btns[1][1]['text']== game_btns[2][2]['text'] != "":
         game_btns[0][0].config(bg='gold')
         game_btns[1][1].config(bg='gold')
         game_btns[2][2].config(bg='gold')
         return True
    #other diagonal 
    elif game_btns[0][2]['text']==game_btns[1][1]['text'] == game_btns[2][0]['text']  != "":
         game_btns[0][2].config(bg='gold')
         game_btns[1][1].config(bg='gold')
         game_btns[2][0].config(bg='gold')
         return True
      #if there are no empty spaces left 
    if check_empty_spaces() == False:
        for row in range(3):
            for col in range (3):
                game_btns[col][row] .config(bg = 'brown')
        return 'tie'
    else:
        return False            
...
window =Tk()
window.title("a7la game to agmal Dr")
